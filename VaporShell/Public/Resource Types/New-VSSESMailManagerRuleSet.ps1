function New-VSSESMailManagerRuleSet {
    <#
    .SYNOPSIS
        Adds an AWS::SES::MailManagerRuleSet resource to the template. 
    ...
    .FUNCTIONALITY
        Vaporshell
    #>
    [OutputType('Vaporshell.Resource.SES.MailManagerRuleSet')]
    [cmdletbinding()]
    Param
    (
        ...
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.SES.MailManagerRuleSet.Rule"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.SES.MailManagerRuleSet'
