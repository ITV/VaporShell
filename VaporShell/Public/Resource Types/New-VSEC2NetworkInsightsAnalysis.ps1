function New-VSEC2NetworkInsightsAnalysis {
    ...
    [OutputType('Vaporshell.Resource.EC2.NetworkInsightsAnalysis')]
    [CmdletBinding()]
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                'FilterOutArns' {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name FilterOutArns -Value @($FilterOutArns)
                }
                'FilterInArns' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name FilterInArns -Value @($FilterInArns)
                }
                'AdditionalAccounts' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name AdditionalAccounts -Value @($AdditionalAccounts)
                }
                'Tags' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.EC2.NetworkInsightsAnalysis'
