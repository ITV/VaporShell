function New-VSSecurityLakeSubscriber {
    <#
    .SYNOPSIS
        Adds an AWS::SecurityLake::Subscriber resource to the template. 
    ...
    .FUNCTIONALITY
        Vaporshell
    #>
    [OutputType('Vaporshell.Resource.SecurityLake.Subscriber')]
    [cmdletbinding()]
    Param
    (
        ...
        [parameter(Mandatory = $true)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.SecurityLake.Subscriber.Source"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                AccessTypes {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name AccessTypes -Value @($AccessTypes)
                }
                Sources {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.SecurityLake.Subscriber'
