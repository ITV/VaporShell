function New-VSRoboMakerRobotApplication {
    ...
    [OutputType('Vaporshell.Resource.RoboMaker.RobotApplication')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.RoboMaker.RobotApplication.SourceConfig"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.RoboMaker.RobotApplication'
