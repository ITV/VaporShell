...
Workforces can be created using Amazon Cognito or your own OIDC Identity Provider IdP. For private workforces created using Amazon Cognito use CognitoMemberDefinition. For workforces created using your own OIDC identity provider IdP use OidcMemberDefinition.
    ...
    .FUNCTIONALITY
        Vaporshell
    #>
    [OutputType('Vaporshell.Resource.SageMaker.Workteam')]
    [cmdletbinding()]
    Param
    (
        ...
        [parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.SageMaker.Workteam.MemberDefinition"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                MemberDefinitions {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name MemberDefinitions -Value @($MemberDefinitions)
                }
                Tags {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.SageMaker.Workteam'
