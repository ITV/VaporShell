function New-VSCassandraType {
    <#
    .SYNOPSIS
        Adds an AWS::Cassandra::Type resource to the template.
    ...
    [OutputType('Vaporshell.Resource.Cassandra.Type')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $true)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.Cassandra.Type.Field"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.Cassandra.Type'
