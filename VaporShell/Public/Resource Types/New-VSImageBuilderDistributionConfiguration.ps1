function New-VSImageBuilderDistributionConfiguration {
    ...
    [OutputType('Vaporshell.Resource.ImageBuilder.DistributionConfiguration')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $true)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.ImageBuilder.DistributionConfiguration.Distribution"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                Distributions {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name Distributions -Value @($Distributions)
                }
                Default {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.ImageBuilder.DistributionConfiguration'
