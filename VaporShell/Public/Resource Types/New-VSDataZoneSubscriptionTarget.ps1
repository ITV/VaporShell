function New-VSDataZoneSubscriptionTarget {
    <#
    .SYNOPSIS
        Adds an AWS::DataZone::SubscriptionTarget resource to the template.
    ...
    [OutputType('Vaporshell.Resource.DataZone.SubscriptionTarget')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $true)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.DataZone.SubscriptionTarget.SubscriptionTargetForm"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                SubscriptionTargetConfig {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name SubscriptionTargetConfig -Value @($SubscriptionTargetConfig)
                }
                ApplicableAssetTypes {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name ApplicableAssetTypes -Value @($ApplicableAssetTypes)
                }
                AuthorizedPrincipals {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name AuthorizedPrincipals -Value @($AuthorizedPrincipals)
                }
                Default {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.DataZone.SubscriptionTarget'
