function New-VSEventSchemasRegistry {
    ...
    [OutputType('Vaporshell.Resource.EventSchemas.Registry')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.EventSchemas.Registry.TagsEntry"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.EventSchemas.Registry'
