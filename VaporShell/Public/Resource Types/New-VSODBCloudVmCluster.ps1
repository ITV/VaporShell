function New-VSODBCloudVmCluster {
    <#
    .SYNOPSIS
        Adds an AWS::ODB::CloudVmCluster resource to the template.
    ...
    .PARAMETER DbNodes
        Documentation: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-odb-cloudvmcluster.html#cfn-odb-cloudvmcluster-dbnodes
    ...
    [OutputType('Vaporshell.Resource.ODB.CloudVmCluster')]
    [CmdletBinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.ODB.CloudVmCluster.DbNode"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                'DbServers' {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name DbServers -Value @($DbServers)
                }
                'DbNodes' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name DbNodes -Value @($DbNodes)
                }
                'Tags' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.ODB.CloudVmCluster'
