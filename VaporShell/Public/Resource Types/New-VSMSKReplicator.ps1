function New-VSMSKReplicator {
    <#
    .SYNOPSIS
        Adds an AWS::MSK::Replicator resource to the template.
    ...
    [OutputType('Vaporshell.Resource.MSK.Replicator')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $true)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.MSK.Replicator.ReplicationInfo"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        [Parameter(Mandatory = $true)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.MSK.Replicator.KafkaCluster"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                ReplicationInfoList {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name ReplicationInfoList -Value @($ReplicationInfoList)
                }
                KafkaClusters {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name KafkaClusters -Value @($KafkaClusters)
                }
                Tags {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.MSK.Replicator'
