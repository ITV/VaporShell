function New-VSLexBotAlias {
    ...
    [OutputType('Vaporshell.Resource.Lex.BotAlias')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.Lex.BotAlias.BotAliasLocaleSettingsItem"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                BotAliasLocaleSettings {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name BotAliasLocaleSettings -Value @($BotAliasLocaleSettings)
                }
                BotAliasTags {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name BotAliasTags -Value @($BotAliasTags)
                }
                Default {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.Lex.BotAlias'
