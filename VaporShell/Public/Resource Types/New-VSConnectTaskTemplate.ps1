function New-VSConnectTaskTemplate {
    <#
    .SYNOPSIS
        Adds an AWS::Connect::TaskTemplate resource to the template.
    ...
    .PARAMETER SelfAssignContactFlowArn
    ...
    [OutputType('Vaporshell.Resource.Connect.TaskTemplate')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.Connect.TaskTemplate.DefaultFieldValue"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.Connect.TaskTemplate.Field"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                Defaults {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name Defaults -Value @($Defaults)
                }
                Fields {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.Connect.TaskTemplate'
