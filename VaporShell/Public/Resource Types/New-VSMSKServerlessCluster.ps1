function New-VSMSKServerlessCluster {
    <#
    .SYNOPSIS
        Adds an AWS::MSK::ServerlessCluster resource to the template. 
    ...
    .FUNCTIONALITY
        Vaporshell
    #>
    [OutputType('Vaporshell.Resource.MSK.ServerlessCluster')]
    [cmdletbinding()]
    Param
    (
        ...
        [parameter(Mandatory = $true)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.MSK.ServerlessCluster.VpcConfig"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                VpcConfigs {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name VpcConfigs -Value @($VpcConfigs)
                }
                Default {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.MSK.ServerlessCluster'
