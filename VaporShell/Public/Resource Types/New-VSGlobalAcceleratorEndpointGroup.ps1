function New-VSGlobalAcceleratorEndpointGroup {
    ...
    [OutputType('Vaporshell.Resource.GlobalAccelerator.EndpointGroup')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.GlobalAccelerator.EndpointGroup.EndpointConfiguration"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.GlobalAccelerator.EndpointGroup'
