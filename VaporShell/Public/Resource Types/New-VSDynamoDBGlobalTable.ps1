function New-VSDynamoDBGlobalTable {
    ...
    .PARAMETER GlobalTableWitnesses
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.DynamoDB.GlobalTable.GlobalTableWitness"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                'GlobalTableWitnesses' {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name GlobalTableWitnesses -Value @($GlobalTableWitnesses)
                }
                'AttributeDefinitions' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
