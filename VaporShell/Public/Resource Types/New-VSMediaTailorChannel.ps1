function New-VSMediaTailorChannel {
    <#
    .SYNOPSIS
        Adds an AWS::MediaTailor::Channel resource to the template. 
    ...
    .FUNCTIONALITY
        Vaporshell
    #>
    [OutputType('Vaporshell.Resource.MediaTailor.Channel')]
    [cmdletbinding()]
    Param
    (
        ...
        [parameter(Mandatory = $true)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.MediaTailor.Channel.RequestOutputItem"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                Audiences {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name Audiences -Value @($Audiences)
                }
                Outputs {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.MediaTailor.Channel'
