function New-VSEntityResolutionIdMappingWorkflow {
    <#
    .SYNOPSIS
        Adds an AWS::EntityResolution::IdMappingWorkflow resource to the template.
    ...
    .PARAMETER IdMappingIncrementalRunConfig
        Documentation: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-entityresolution-idmappingworkflow.html#cfn-entityresolution-idmappingworkflow-idmappingincrementalrunconfig
        UpdateType: Mutable
    ...
    [OutputType('Vaporshell.Resource.EntityResolution.IdMappingWorkflow')]
    [CmdletBinding()]
    ...
    (
        ...
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.EntityResolution.IdMappingWorkflow.IdMappingWorkflowInputSource"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.EntityResolution.IdMappingWorkflow.IdMappingWorkflowOutputSource"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                'InputSourceConfig' {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name InputSourceConfig -Value @($InputSourceConfig)
                }
                'OutputSourceConfig' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name OutputSourceConfig -Value @($OutputSourceConfig)
                }
                'Tags' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.EntityResolution.IdMappingWorkflow'
