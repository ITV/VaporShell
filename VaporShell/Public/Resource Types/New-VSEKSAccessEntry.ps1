function New-VSEKSAccessEntry {
    <#
    .SYNOPSIS
        Adds an AWS::EKS::AccessEntry resource to the template.
    ...
    [OutputType('Vaporshell.Resource.EKS.AccessEntry')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.EKS.AccessEntry.AccessPolicy"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                KubernetesGroups {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name KubernetesGroups -Value @($KubernetesGroups)
                }
                AccessPolicies {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name AccessPolicies -Value @($AccessPolicies)
                }
                Tags {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.EKS.AccessEntry'
