function New-VSRbinRule {
    <#
    .SYNOPSIS
        Adds an AWS::Rbin::Rule resource to the template.
    ...
    [OutputType('Vaporshell.Resource.Rbin.Rule')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.Rbin.Rule.ResourceTag"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                ExcludeResourceTags {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name ExcludeResourceTags -Value @($ExcludeResourceTags)
                }
                Tags {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.Rbin.Rule'
