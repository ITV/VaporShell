function New-VSGameLiftGameServerGroup {
    ...
    [OutputType('Vaporshell.Resource.GameLift.GameServerGroup')]
    [cmdletbinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $true)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.GameLift.GameServerGroup.InstanceDefinition"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                VpcSubnets {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name VpcSubnets -Value @($VpcSubnets)
                }
                InstanceDefinitions {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name InstanceDefinitions -Value @($InstanceDefinitions)
                }
                Tags {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.GameLift.GameServerGroup'
