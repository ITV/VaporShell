function New-VSCleanRoomsConfiguredTable {
    <#
    .SYNOPSIS
        Adds an AWS::CleanRooms::ConfiguredTable resource to the template.
    ...
    [OutputType('Vaporshell.Resource.CleanRooms.ConfiguredTable')]
    [CmdletBinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.CleanRooms.ConfiguredTable.AnalysisRule"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
        ...
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                ...
                'SelectedAnalysisMethods' {
                    ...
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name SelectedAnalysisMethods -Value @($SelectedAnalysisMethods)
                }
                'AnalysisRules' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name AnalysisRules -Value @($AnalysisRules)
                }
                'AllowedColumns' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
                    }
                    $ResourceParams["Properties"] | Add-Member -MemberType NoteProperty -Name AllowedColumns -Value @($AllowedColumns)
                }
                'Tags' {
                    if (!($ResourceParams["Properties"])) {
                        $ResourceParams.Add("Properties",([PSCustomObject]@{}))
    ...
    End {
        ...
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.CleanRooms.ConfiguredTable'
