function Add-VSIoTTopicRuleRepublishActionHeaders {
    <#
    .SYNOPSIS
        Adds an AWS::IoT::TopicRule.RepublishActionHeaders resource property to the template. 
    ...
    (
        ...
        [parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.IoT.TopicRule.UserProperty"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.IoT.TopicRule.RepublishActionHeaders'
