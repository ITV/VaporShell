function Add-VSAPSWorkspaceWorkspaceConfiguration {
    <#
    .SYNOPSIS
        Adds an AWS::APS::Workspace.WorkspaceConfiguration resource property to the template.
    ...
    [OutputType('Vaporshell.Resource.APS.Workspace.WorkspaceConfiguration')]
    [CmdletBinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.APS.Workspace.LimitsPerLabelSet"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.APS.Workspace.WorkspaceConfiguration'
