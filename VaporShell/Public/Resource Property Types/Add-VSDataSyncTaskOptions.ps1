function Add-VSDataSyncTaskOptions {
    <#
    .SYNOPSIS
        Adds an AWS::DataSync::Task.Options resource property to the template. Represents the options that are available to control the behavior of a StartTaskExecution: https://docs.aws.amazon.com/datasync/latest/userguide/API_StartTaskExecution.html operation. This behavior includes preserving metadata, such as user ID (UID, group ID (GID, and file permissions; overwriting files in the destination; data integrity verification; and so on.

    .DESCRIPTION
        Adds an AWS::DataSync::Task.Options resource property to the template.
...
AWS DataSync can preserve extant permissions of a source location.
    ...
    .PARAMETER PreserveDeletedFiles
...
PRESERVE: Preserve character and block device metadata. This option isn't currently supported for Amazon EFS.
    ...
    .PARAMETER SecurityDescriptorCopyFlags
...
NONE: No additional verification is done at the end of the transfer, but all data transmissions are integrity-checked with checksum verification during the transfer.
    ...
    End {
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.DataSync.Task.Options'
