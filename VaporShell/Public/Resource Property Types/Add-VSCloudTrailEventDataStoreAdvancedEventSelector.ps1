function Add-VSCloudTrailEventDataStoreAdvancedEventSelector {
    <#
    .SYNOPSIS
        Adds an AWS::CloudTrail::EventDataStore.AdvancedEventSelector resource property to the template. 
    ...
    .FUNCTIONALITY
        Vaporshell
    #>
    [OutputType('Vaporshell.Resource.CloudTrail.EventDataStore.AdvancedEventSelector')]
    [cmdletbinding()]
    Param
    (
        ...
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.CloudTrail.EventDataStore.AdvancedFieldSelector"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.CloudTrail.EventDataStore.AdvancedEventSelector'
