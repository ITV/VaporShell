function Add-VSWAFv2RuleGroupTextTransformation {
    <#
    .SYNOPSIS
        Adds an AWS::WAFv2::RuleGroup.TextTransformation resource property to the template. Text transformations eliminate some of the unusual formatting that attackers use in web requests in an effort to bypass detection.
...
**ESCAPE_SEQ_DECODE** - Decode the following ANSI C escape sequences: a, b, f, n, r, t, v, , ?, ', ", xHH hexadecimal, 0OOO octal. Encodings that aren't valid remain in the output.
...
**UTF8_TO_UNICODE** - Convert all UTF-8 character sequences to Unicode. This helps input normalization, and minimizing false-positives and false-negatives for non-English languages.
    ...
    End {
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.WAFv2.RuleGroup.TextTransformation'
