function Add-VSIoTTwinMakerComponentTypeDataValue {
    <#
    .SYNOPSIS
        Adds an AWS::IoTTwinMaker::ComponentType.DataValue resource property to the template. 
    ...
    .FUNCTIONALITY
        Vaporshell
    #>
    [OutputType('Vaporshell.Resource.IoTTwinMaker.ComponentType.DataValue')]
    [cmdletbinding()]
    Param
    (
        ...
        [parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.IoTTwinMaker.ComponentType.DataValue"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.IoTTwinMaker.ComponentType.DataValue'
