function Add-VSCloudFrontTrustStoreCaCertificatesBundleSource {
    <#
    .SYNOPSIS
        Adds an AWS::CloudFront::TrustStore.CaCertificatesBundleSource resource property to the template.

    .DESCRIPTION
        Adds an AWS::CloudFront::TrustStore.CaCertificatesBundleSource resource property to the template.

    .LINK
        http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-cloudfront-truststore-cacertificatesbundlesource.html

    .PARAMETER CaCertificatesBundleS3Location
        Documentation: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-cloudfront-truststore-cacertificatesbundlesource.html#cfn-cloudfront-truststore-cacertificatesbundlesource-cacertificatesbundles3location
        UpdateType: Mutable
        Type: CaCertificatesBundleS3Location

    .FUNCTIONALITY
        Vaporshell
    #>

    [OutputType('Vaporshell.Resource.CloudFront.TrustStore.CaCertificatesBundleSource')]
    [CmdletBinding()]

    Param
    (
        [Parameter(Mandatory = $true)]
        $CaCertificatesBundleS3Location

    )

    Begin {
        $obj = [PSCustomObject]@{}
        $commonParams = @('Verbose','Debug','ErrorAction','WarningAction','InformationAction','ErrorVariable','WarningVariable','InformationVariable','OutVariable','OutBuffer','PipelineVariable')
    }

    Process {
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                Default {
                    $obj | Add-Member -MemberType NoteProperty -Name $key -Value $PSBoundParameters.$key
                }
            }
        }
    }

    End {
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.CloudFront.TrustStore.CaCertificatesBundleSource'
        Write-Verbose "Resulting JSON from $($MyInvocation.MyCommand): `n`n$($obj | ConvertTo-Json -Depth 5)`n"
    }
}
