function Add-VSBedrockPromptVersionPromptVariant {
    <#
    .SYNOPSIS
        Adds an AWS::Bedrock::PromptVersion.PromptVariant resource property to the template.
    ...
    [OutputType('Vaporshell.Resource.Bedrock.PromptVersion.PromptVariant')]
    [CmdletBinding()]
    ...
    (
        ...
        [Parameter(Mandatory = $false)]
        [ValidateScript( {
                $allowedTypes = "Vaporshell.Resource.Bedrock.PromptVersion.PromptMetadataEntry"
                if ([string]$($_.PSTypeNames) -match "($(($allowedTypes|ForEach-Object{[RegEx]::Escape($_)}) -join '|'))") {
                    $true
                }
                else {
    ...
    End {
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.Bedrock.PromptVersion.PromptVariant'
