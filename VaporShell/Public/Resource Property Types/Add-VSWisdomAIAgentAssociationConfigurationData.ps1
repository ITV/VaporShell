function Add-VSWisdomAIAgentAssociationConfigurationData {
    <#
    .SYNOPSIS
        Adds an AWS::Wisdom::AIAgent.AssociationConfigurationData resource property to the template.

    .DESCRIPTION
        Adds an AWS::Wisdom::AIAgent.AssociationConfigurationData resource property to the template.

    .LINK
        http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-wisdom-aiagent-associationconfigurationdata.html

    .PARAMETER KnowledgeBaseAssociationConfigurationData
        Documentation: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-wisdom-aiagent-associationconfigurationdata.html#cfn-wisdom-aiagent-associationconfigurationdata-knowledgebaseassociationconfigurationdata
        UpdateType: Mutable
        Type: KnowledgeBaseAssociationConfigurationData

    .FUNCTIONALITY
        Vaporshell
    #>

    [OutputType('Vaporshell.Resource.Wisdom.AIAgent.AssociationConfigurationData')]
    [cmdletbinding()]

    Param
    (
        [Parameter(Mandatory = $true)]
        $KnowledgeBaseAssociationConfigurationData

    )

    Begin {
        $obj = [PSCustomObject]@{}
        $commonParams = @('Verbose','Debug','ErrorAction','WarningAction','InformationAction','ErrorVariable','WarningVariable','InformationVariable','OutVariable','OutBuffer','PipelineVariable')
    }

    Process {
        foreach ($key in $PSBoundParameters.Keys | Where-Object {$commonParams -notcontains $_}) {
            switch ($key) {
                Default {
                    $obj | Add-Member -MemberType NoteProperty -Name $key -Value $PSBoundParameters.$key
                }
            }
        }
    }

    End {
        $obj | Add-ObjectDetail -TypeName 'Vaporshell.Resource.Wisdom.AIAgent.AssociationConfigurationData'
        Write-Verbose "Resulting JSON from $($MyInvocation.MyCommand): `n`n$($obj | ConvertTo-Json -Depth 5)`n"
    }
}
